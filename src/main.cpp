    ...
    while (run) {
        ...
        // destroy a VAO + VBO on every single frame.
        {
            ...
            if (state.showGameObjects && !g_GameObjects.empty()) {
                ...
                std::vector<glm::vec3> savePoints;
                for (const auto& go : g_GameObjects) {
                    if (go.className == "SavePoint") {
                        // Create a floating red diamond that spins
                        float t = (float)ImGui::GetTime() * 2.0f;
                        glm::mat4 rot = glm::rotate(glm::mat4(1.0f), t, glm::vec3(0,1,0));
                        glm::vec3 top = go.position + glm::vec3(0.0f, 1.5f + sinf(t)*0.1f, 0.0f);
                        glm::vec3 p1 = go.position + glm::vec3(0.0f, 1.0f + sinf(t)*0.1f, 0.0f) + glm::vec3(rot * glm::vec4(0.3f, 0.0f, 0.3f, 1.0f));
                        glm::vec3 p2 = go.position + glm::vec3(0.0f, 1.0f + sinf(t)*0.1f, 0.0f) + glm::vec3(rot * glm::vec4(-0.3f, 0.0f, 0.3f, 1.0f));
                        glm::vec3 p3 = go.position + glm::vec3(0.0f, 1.0f + sinf(t)*0.1f, 0.0f) + glm::vec3(rot * glm::vec4(0.0f, 0.0f, -0.4f, 1.0f));
                        glm::vec3 bot = go.position + glm::vec3(0.0f, 0.5f + sinf(t)*0.1f, 0.0f);
                        savePoints.push_back(top); savePoints.push_back(p1);
                        savePoints.push_back(top); savePoints.push_back(p2);
                        savePoints.push_back(top); savePoints.push_back(p3);
                        savePoints.push_back(bot); savePoints.push_back(p1);
                        savePoints.push_back(bot); savePoints.push_back(p2);
                        savePoints.push_back(bot); savePoints.push_back(p3);
                        savePoints.push_back(p1); savePoints.push_back(p2);
                        savePoints.push_back(p2); savePoints.push_back(p3);
                        savePoints.push_back(p3); savePoints.push_back(p1);
                        continue;
                    }
                ...
                drawBatch(savePoints, 1.0f, 0.1f, 0.1f); // Bright red
